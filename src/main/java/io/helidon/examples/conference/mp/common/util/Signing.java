 ...
 * @version 1.0.1
 *
 *          This example creates a {@link RequestSigner}, then prints out the
 *          Authorization header that is inserted into the HttpGet object.
 *
 *          <p>
 *          apiKey is the identifier for a key uploaded through the console.
 *          privateKeyFilename is the location of your private key (that matches
 *          the uploaded public key for apiKey).
 *          </p>
 *
 *          The signed HttpGet request is not executed, since instanceId does
 *          not map to a real instance.
 */
public class Signing {
	public static void main(String[] args) throws UnsupportedEncodingException {
		...
		String apiKey = ("ocid1.tenancy.oc1..aaaaaaaa3mb7wrcy2ls3u3jsy2soq5ck3lc3q4mczitpdaymbuazc5tkguca" + "/"
				+ "ocid1.user.oc1..aaaaaaaaalcctmnqefychuoqhmdp4hcfpzwlgpakgtlnlu43gd7wrie56s6q" + "/"
				+ "33:be:4a:4b:ed:0d:1e:e2:cc:d4:f6:59:22:88:b5:dd");
		String privateKeyFilename = "/home/oracle/.oci/private.pem";
		...
		uri = String.format(uri, "Pjwf%3A%20PHX-AD-1",
				...
				"ocid1.compartment.oc1..aaaaaaaam3we6vgnherjq5q2idnccdflvjsnog7mlr6rtdb25gilchfeyjxa".replace(":",
						"%3A"),
				"TeamXInstances", "ocid1.volume.oc1.phx.abyhqljrgvttnlx73nmrwfaux7kcvzfs3s66izvxf2h4lgvyndsdsnoiwr5q"
						.replace(":", "%3A"));
	...
	public static class RequestSigner {
		...
		public RequestSigner(String apiKey, Key privateKey) {
			this.signers = REQUIRED_HEADERS.entrySet().stream().collect(Collectors.toMap(entry -> entry.getKey(),
		...
		protected Signer buildSigner(String apiKey, Key privateKey, String method) {
			final Signature signature = new Signature(apiKey, SIGNATURE_ALGORITHM, null,
...
//		public void signHttpRequest(HttpRequestBase request, HttpRequest httpRequest) {
//
//			signRequest(request);
//
//			final String method = httpRequest.getHttpMethod().toString().toLowerCase();
//			Map<String, List<String>> getRequestHeaders = httpRequest.getHeaders();
//			// nothing to sign for options
//			if (method.equals("options")) {
//				return;
//			}
//			final String path = extractPath(request.getURI());
//			// supply date if missing
//			if (getRequestHeaders.get("date") == null) {
//				httpRequest.header("date", DATE_FORMAT.format(new Date()));
//			}
//			// supply host if mossing
//			if (getRequestHeaders.get("host") == null) {
//				httpRequest.header("host", request.getURI().getHost());
//			}
//			// supply content-type, content-length, and x-content-sha256 if missing (PUT and
//			// POST only)
//			if (method.equals("put") || method.equals("post")) {
//				if (getRequestHeaders.get("content-type") == null) {
//					httpRequest.header("content-type", "application/json");
//				}
//				if (getRequestHeaders.get("content-length") == null
//						|| getRequestHeaders.get("x-content-sha256") == null) {
////					RawBody rawBody = new RawBody(httpRequest);
////					byte[] body = (byte[])((RawBody)rawBody.getHttpRequest().getBody()).getBody();
//					byte[] body = getRequestBody((HttpEntityEnclosingRequestBase) request);
//					if (getRequestHeaders.get("content-length") == null) {
//						httpRequest.header("content-length", Integer.toString(body.length));
//					}
//					if (getRequestHeaders.get("x-content-sha256") == null) {
//						httpRequest.header("x-content-sha256", calculateSHA256(body));
//					}
//				}
//			}
//			final Map<String, String> headers = extractHeadersToSign(request);
//			final String signature = this.calculateSignature(method, path, headers);
//			httpRequest.header("Authorization", signature);
//		}
		...
		private static Map<String, String> extractHeadersToSign(HttpRequestBase request) {
			...
			return headersToSign.stream()
					...
					.collect(Collectors.toMap(header -> header, header -> {

//						if ("content-length".contentEquals(header)) {
//							return "";
//						}
