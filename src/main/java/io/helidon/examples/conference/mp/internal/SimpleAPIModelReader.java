 ...
 *
 */
package io.helidon.examples.conference.mp.internal;
...
public class SimpleAPIModelReader implements OASModelReader {
	...
	/**
	 * Path for the example endpoint added by this model reader that should be
	 * visible.
	 */
	...
	public OpenAPI buildModel() {
		/*
		 * Add two path items, one of which we expect to be removed by the filter and a
		 * very simple one that will appear in the published OpenAPI document.
		 */
		PathItem newPathItem = OASFactory.createPathItem()
				.GET(OASFactory.createOperation().operationId("newPath").summary(SUMMARY));
		PathItem doomedPathItem = OASFactory.createPathItem().GET(
				OASFactory.createOperation().operationId(DOOMED_OPERATION_ID).summary("This should become invisible"));
		OpenAPI openAPI = OASFactory.createOpenAPI();
		Paths paths = OASFactory.createPaths().addPathItem(MODEL_READER_PATH, newPathItem).addPathItem(DOOMED_PATH,
				doomedPathItem);
		openAPI.paths(paths);
